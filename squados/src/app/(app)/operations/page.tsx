    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Fluxo Produtivo</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap items-center gap-2">
            {PRODUCTION_FLOW.map((step, i) => {
              const stats = getSectorStats(step.slug);
              const hasKnowledge = stats.docs > 0;
              return (
                <div key={step.slug} className="flex items-center gap-2">
                  <div className="text-center">
                    <div className={`relative px-3 py-2 rounded-lg border-2 transition-all ${
                      ...
                      <span className="text-[10px] font-bold text-muted-foreground block">{step.step}</span>
                      <span className="text-xs font-semibold block">{step.label}</span>
                      <div className="flex items-center gap-1.5 mt-1 justify-center">
                        {stats.users > 0 && (
                          <span className="text-[9px] text-muted-foreground flex items-center gap-0.5">
                            <Users className="w-2.5 h-2.5" />{stats.users}
                          </span>
                        )}
                        {stats.docs > 0 && (
                          <span className="text-[9px] text-muted-foreground flex items-center gap-0.5">
                            <FileText className="w-2.5 h-2.5" />{stats.docs}
                          </span>
                        )}
                        {stats.hasAgent && (
